...
Inhalte für Lern Pfade von Microsoft 365 werden von einer Reihe von JSON-Dateien gesteuert, die als Inhalts Manifeste für Ihr Lern Paket fungieren. Es gibt drei Dateien: Metadata. JSON, Playlisten. JSON und Assets. JSON. Diese Dateien müssen so strukturiert sein, dass Sie den Modellen entsprechen, die das Webpart erkennt und dann von einem Content Delivery Network (CDN) gehostet wird, damit das Webpart Sie laden kann. Microsoft stellt Start Vorlagen dieser Dateien bereit, damit Sie den Einstieg erhalten.  
...
|Image                         |Relative URL (von CDN) zu einem Bild, um die Wiedergabeliste zu visualisieren                              |                      
...
|Quelle                        |Aus dem Quell-Array, nicht speziell in UX andere als benutzerdefinierte Daten, die vom Benutzer hinzugefügt werden, wird als "Mandant" und der UX-Admin-Bereich nicht erlaubt die Bearbeitung von nichts, das nicht als "Mandant" gekennzeichnet.                                              |
